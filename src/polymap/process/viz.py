...
def make_study_plot(pl: ProcessLayouts, pgp: ProcessGraphPairs):
    ...
    plot_layout(pl.simplified, "simplified", ax1, show=False, add_labels=False)

    plot_graph_pairs_and_layout(
        pl.simplified, "xgraph", pgp.x, ax2, alpha=ALPHA, add_labels=False
    )
    plot_layout(pl.xpull, "xpull", ax3, show=False, add_labels=True)
